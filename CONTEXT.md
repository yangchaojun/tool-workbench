...
一个纯前端、本地优先的工具集合。每个工具自包含于 `src/tools/<id>/`，通过注册表接入导航与路由；所有数据只留在浏览器内（`tw:` 前缀的 localStorage），平台默认不发起任何网络请求——工具唯有经显式声明才可提供**显式网络加载**能力（见 ADR-0004）。
...
**显式网络加载（User-initiated Network Load）**:
工具经显式声明后获得的网络能力：仅限用户主动触发的一次性请求，平台默认仍无任何网络行为。
_Avoid_: 在线模式、联网功能、API 集成
...
**远程加载（Remote Load）**:
由用户点击触发、对目标 URL 发起单次 GET 并把响应体作为输入载入的动作；不携带自定义请求头，跨域受限时如实提示。
_Avoid_: 抓取、爬取、在线导入、fetch

**文档历史（Document History）**:
跨会话持久化的已载入文档记录（来源与内容），供随时恢复；与撤销/重做的统一历史栈无关。
_Avoid_: 历史记录、最近文件、recent files
...
**软上限（Soft Cap）**:
超出即警告但不阻止继续使用的规模阈值（输入 1MB、上传 20MB、持久化 256KB）。
